...
chrome.notifications.onButtonClicked.addListener(async (notificationId, buttonIndex) => {
  ...
  if (buttonIndex === 0) {
    ...
    // Local update will happen via Firestore snapshot listener if it was active, 
    // but background script might not have one for completions.
    // Let's at least update local storage so UI is snappy.
    const { completions } = await loadHabitsAndCompletions();
    if (!Array.isArray(completions[todayKey])) completions[todayKey] = [];
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    // Subscribe to habits to keep alarms in sync
    const q = query(collection(db, 'habits'), where('userId', '==', user.uid));
    habitsUnsubscribe = onSnapshot(q, (snapshot) => {
      const habits = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      chrome.storage.local.set({ [STORAGE_KEY_HABITS]: habits });
      scheduleHabitAlarms();
    });
  } else {
